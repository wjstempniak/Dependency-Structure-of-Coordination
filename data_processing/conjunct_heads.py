...
def coordination(sent, heads, lang_params, corpus):
    # Heads
    Lhead = heads[0]
    Rhead = heads[-1]
    ...
    # Governor
    G = head(sent, Lhead)
    G_id = "" if not G else G['id']
    C_id = "" if not C else C['id']

    #print(sent.metadata["text"])

    coord_data = [G_id, C_id,
                  Lhead["id"], L[0]['id'], L[-1]['id'],
                  Rhead["id"], R[0]['id'], R[-1]['id'],
            ...
            for c in children(sent, head):
                if c["deprel"] == "cc" and c["form"] != C["form"]:  # A & B / C
                    ...
                    coords += [coordination(sent, heads[0:h],
                                            lang_params, corpus)]  # [A] & [B]
                    coords += [coordination(sent, [heads[0]] + heads[h:],
                                            lang_params, corpus)]  # [A & B] / [C]
                    return coords

    return [coordination(sent, heads, lang_params, corpus)]
...
def analyse(input, output, lang_params, corpus):
    with open(input, "r", encoding="utf-8") as infile, \
        ...
        header = ["governor.id", "conj.id",
                  "L.head.id", "L.first.id", "L.last.id",
                  "R.head.id", "R.first.id", "R.last.id",
...
def main(folder, languages=None):
    ...
    for language in os.listdir(folder):
        ...
        output_folder = folder + '/' + language + '/heads'
